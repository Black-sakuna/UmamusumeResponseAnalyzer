...
* `manifest.json` 必须精确包含 `Author`、`InternalName`、`DisplayName`、`Description`、`Changelog`、`Version`、`Dependencies`、`Targets`、`RepositoryUrl`、`LastUpdate`、`Category`、`Homepage`。`Dependencies` 声明直接依赖的插件 internal name；宿主按依赖顺序加载、按反向顺序卸载，并让同一依赖连通组共享 load context。缺失依赖、自依赖、重复名称和依赖环均使插件包加载失败。`Targets` 为空、命中配置的 repository targets，或配置未设置 targets 时，宿主才注册该插件的 analyzer。
...
* 热重载按 manifest `InternalName` 应用；有依赖关系的插件按 manifest 依赖连通组一起重载。
