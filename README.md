...
* 选择 `启动！` 后会立即进入 Terminal.Gui 的全屏启动 workspace；数据加载、插件初始化和 HTTP server 启动在后台推进。页面固定占满 terminal viewport，运行环境、初始化结果、插件摘要和最近日志四个区域的尺寸只随 viewport 变化；超出区域的表格与日志使用 Terminal.Gui 原生滚动条，内容增长不改变 panel 布局。启动状态以紧凑结果行实时更新，不显示额外 header 或 footer。
