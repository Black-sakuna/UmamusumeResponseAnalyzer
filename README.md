...
* `manifest.json` 必须精确包含 `Author`、`InternalName`、`DisplayName`、`Description`、`Changelog`、`Version`、`Dependencies`、`Targets`、`RepositoryUrl`、`LastUpdate`、`Category`、`Homepage`。`Dependencies` 声明直接软联动的插件 internal name：目标缺失时忽略该边且 Consumer 独立加载；双方存在时宿主按声明顺序先初始化目标插件、按反向顺序卸载，并让依赖连通组共享 collectible load context。已安装图中的自依赖、重复名称和依赖环使插件加载失败。Consumer 通过 `IPluginContext.IsPluginAvailable` 判断本轮共享组中的声明目标；可选插件类型不得出现在 Consumer 导出类型签名或启动必经字段初始化中。`Targets` 为空、命中配置的 repository targets，或配置未设置 targets 时，宿主才注册该插件的 analyzer。
